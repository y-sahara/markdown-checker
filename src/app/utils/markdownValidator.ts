...
import { ValidationResult, ValidationCategory, ValidationOptions } from '../types';
...
export const validateMarkdown = async (
  ...
  try {
    ...
    const results = file.messages.map((message: any) => {
      // メッセージからカテゴリを判断
      ...
      } else if (ruleId.includes('strikethrough') || ruleId.includes('delete')) {
      ...
      return {
        line: message.position?.start?.line || 0,
        column: message.position?.start?.column || 0,
    ...
    return options.categories.includes(ValidationCategory.GENERAL)
      ? results
      : results.filter(result => options.categories.includes(result.category));
    ...
    return [{
      line: 0,
      column: 0,
      message: `パース中にエラーが発生しました: ${error.message}`,
      ruleId: 'parse-error',
      severity: 'error',
